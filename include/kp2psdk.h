...
typedef struct kp2p_callback_s {
     ...
     */
    void (*OnVconRecv)(kp2p_handle_t p2p_handle, void *context, uint32_t channel, const void *data, size_t data_size);
    ...
    /**
     * @brief KP2P 日志上报事件，需要调用阿里的日志库进行上报（https://help.aliyun.com/product/28958.html?spm=a2c4g.750001.3.40.H2MyoP）
     *  服务器已经配置好 PROJECT_NAME: "juan-esee"
     *                LOGSTORE_NAME: "kp2p"
     *                log 为 一系列的 key-value json 字符串，需要解析出来，使用阿里sdk 进行上报。
     *
     */
    void (*OnLogUpload)(kp2p_handle_t p2p_handle, void *context, const char *log, size_t log_len);
} kp2p_callback_t;
...
KP2P_API void kp2p_set_log(int level, const char *path, int max_kbytes);
...
/**
 * @brief 创建p2p会话句柄，使用新的协议，使用k2p_create_v3创建的句柄，在调用其它接口时内部逻辑与kp2p_create的句柄不同
 * @param context 用户上下文
 * @return 成功，返回有效句柄 
 */
KP2P_API kp2p_handle_t kp2p_create_v2(void *context);
...
/**
 * 同上，建立p2p连接，直接返回IOT层的错误号
 */
KP2P_API int kp2p_connect_v2(kp2p_handle_t p2p_handle, const char* dev_id, const char *ipaddr, uint32_t port);
 ...
 *                          失败  -1
 */
KP2P_API int kp2p_vcon_send(kp2p_handle_t p2p_handle, uint32_t channel, const void *data, size_t data_size);
...
KP2P_API int kp2p_reset(void);

/**
 * @brief 获取会话ID
 * @return 0 成功
 */ 
KP2P_API int kp2p_getsessionid(kp2p_handle_t p2p_handle, uint32_t *sessionid);
